...
export const getAllReviews = (spotId) => async (dispatch) => {
    const response = await csrfFetch(`https://airbnb-clone-be-d5zd.onrender.com/api/spots/${spotId}/reviews`);
    if (response.ok) {
...
export const deleteReview = (reviewId) => async (dispatch) => {
    const response = await csrfFetch(`https://airbnb-clone-be-d5zd.onrender.com/api/reviews/${reviewId}`, {
        method: "DELETE", 
    })
    if (response.ok) {
