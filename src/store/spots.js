...
export const getAllSpots = () => async (dispatch) => {
    const response = await fetch("https://airbnb-clone-be-d5zd.onrender.com/api/csrf/restore/api/spots");
    if (response.ok) {
  ...
  export const getSpotDetails = (spotId) => async (dispatch) => {
    const response = await csrfFetch(`https://airbnb-clone-be-d5zd.onrender.com/api/csrf/restore/api/spots/${spotId}`);
    if (response.ok) {
  ...
  export const getUserSpots = () => async (dispatch) => {
    const response = await csrfFetch("https://airbnb-clone-be-d5zd.onrender.com/api/csrf/restore/api/spots/current");
    if (response.ok) {
  ...
  export const addNewSpot = (spot, navigate) => async () => {
    const response = await csrfFetch(`https://airbnb-clone-be-d5zd.onrender.com/api/csrf/restore/api/spots`, {
      method: "POST",
  ...
  export const addSpotImage = (spotId, image) => async (dispatch) => {
    const response = await csrfFetch(`https://airbnb-clone-be-d5zd.onrender.com/api/csrf/restore/api/spots/${spotId}/images`, {
      method: "POST",
      headers: {
